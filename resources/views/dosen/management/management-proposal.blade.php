...
<body class="d-flex flex-column min-vh-100">
    ...
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1 class="me-3 mb-0">Management Proposal</h1>
            <form method="GET" action="{{ route('management-proposal') }}" class="d-flex gap-2">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @forelse ($mapels as $mapel)
                            <tr>
                                <form method="POST" action="{{ route('proposal.setDeadline') }}">
                                    @csrf
                                    <input type="hidden" name="mapel_id" value="{{ $mapel->id }}">
                                    <td>{{ $mapel->kelas }}</td>
                                    <td>{{ $mapel->tahun_akademik }}</td>
                                    <td>
                                        <input type="date" name="batas_pengajuan" class="form-control" value="{{ $mapel->batas_pengajuan }}" required>
                                    </td>
                                    <td>
                    ...
                    <tbody>
                        @foreach($proposals as $proposal)
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalDetail{{ $proposal->id }}">
                                        <i class="bi bi-eye"></i>
                                    </button>
                                </td>
                                <td>
                                ...
                                <td>
                                    <button class="btn btn-outline-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#modalComment{{ $proposal->id }}">
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <p>{{ strip_tags($proposal->abstract) }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="modal-dialog modal-dialog-scrollable">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            @endif
                                            {{-- Form Komentar --}}
                                            <form method="POST" action="{{ route('proposal.addComment') }}">
                                                @csrf
                                                ...
                                                <div class="d-flex justify-content-end">
                                                    <button type="submit" class="btn btn-primary">
                                                        <i class="bi bi-send"></i> Simpan Komentar
                                                    </button>
                                                </div>
                                            </form>
                                        </div>
    ...
    <script>
        $(document).ready(function () {
            $('.update-status').change(function () {
                ...
                $.post("{{ route('proposal.updateStatus') }}", {
                    _token: "{{ csrf_token() }}",
                    id, status
    ...
    {{-- Buka modal otomatis jika komentar berhasil disimpan --}}
