...
<body>
    ...
    <div class="container my-4">
        <div class="d-flex align-items-center mb-3">
            <h1 class="me-3 mb-0">Edit Penilaian Penguji 1</h1>
        </div>
    ...
    <form action="{{ route('dosen-pembimbing.updatePenguji1', $mahasiswa->id) }}" method="POST">
    @csrf
    @method('PUT')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="sikap_kp" value ="{{ $mahasiswa->sikap_penguji1/5 }}" class="form-control" required>
        </div>
        <div class="mb-3">
            <label class="form-label">Mampu menjelaskan alasan pengambilan topik penelitian. <b> 5 (Bobot)</b></label>
            <input type="number" name="mampu_menjelaskan_topik_kp" value="{{ $mahasiswa->mampu_menjelaskan_topik_penguji1/5 }}" class="form-control" required>
        </div>
        <div class="mb-3">
            <label class="form-label">Mampu menjelaskan setiap bagian/menu produk atau hasil. <b> 15 (Bobot)</b></label>
            <input type="number" name="mampu_menjelaskan_hasil_kp" value="{{ $mahasiswa->mampu_menjelaskan_hasil_penguji1/15 }}" class="form-control" required>
        </div>
        <h3>2. Produk</h3>
        <div class="mb-3">
            <label class="form-label">Produk dapat disimulasikan dan atau telah diimplementasikan. <b> 20 (Bobot)</b></label>
            <input type="number" name="simulasi_produk_kp" value="{{ $mahasiswa->simulasi_produk_penguji1/20 }}" class="form-control" required>
        </div>
        <div class="mb-3">
            <label class="form-label">Produk telah diuji dengan baik .<b> 10 (Bobot)</b></label>
            <input type="number" name="pengujian_produk_kp" class="form-control" value="{{ $mahasiswa->pengujian_produk_penguji1/10 }}" required>
        </div>
        <div class="mb-3">
            <label class="form-label">Produk bermanfaat dan mampu menyelesaikan masalah(solutif). <b> 15 (Bobot)</b></label>
            <input type="number" name="produk_bermanfaat_kp" class="form-control" value="{{ $mahasiswa->produk_bermanfaat_penguji1/15 }}"  required>
        </div>
         <div class="mb-3">
            <label class="form-label">Kejelasan proses bisnis dan komplesitas produk. <b> 10 (Bobot)</b></label>
            <input type="number" name="kejelasan_proses_kp" class="form-control" value="{{ $mahasiswa->kejelasan_proses_penguji1/10 }}" required>
        </div>
        <h3>3. Laporan</h3>
        <div class="mb-3">
            <label class="form-label">Susunan laporan sesuai format panduan. <b> 5 (Bobot)</b></label>
            <input type="number" name="susunan_laporan_kp" class="form-control" value="{{ $mahasiswa->susunan_laporan_penguji1/5 }}" required>
        </div>
        <div class="mb-3">
            <label class="form-label">Isi laporan sesuai dengan produk penelitian. <b> 10 (Bobot)</b></label>
            <input type="number" name="isi_laporan_kp" class="form-control" value="{{ $mahasiswa->isi_laporan_penguji1/10 }}" required>
        </div>
         <div class="mb-3">
            <label class="form-label">Kualitas penulisan (gramatika, ejaan, sitasi, penulisan pustaka, dll). <b> 5 (Bobot)</b></label>
            <input type="number" name="kualitas_penulisan_kp" value="{{ $mahasiswa->kualitas_penulisan_penguji1/5 }}" class="form-control" required>
        </div>
