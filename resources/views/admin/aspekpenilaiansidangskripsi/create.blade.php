...
<head>
    ...
    <title>Tambah Aspek Penilaian Sidang HKI</title>
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card shadow-sm">
                    <div class="card-body">
                        <h3 class="card-title mb-4">Tambah Aspek Penilaian Sidang Skripsi</h3>

                        @if(session('success'))
                            <div class="alert alert-success">{{ session('success') }}</div>
                        @endif

                        <form action="{{ route('penilaian.sidang.skripsi.store') }}" method="POST">
                            @csrf

                            <div class="mb-3 row">
                                <label class="col-sm-3 col-form-label">Unsur yang Dinilai</label>
                                <div class="col-sm-9">
                                    <input type="text" name="unsur_yang_dinilai" class="form-control" required>
                                </div>
                            </div>
